...
molecule = [ ["Xe", 54, 0.0, 0.0, 0.0, 0.0 , 0] ]
...
if not one_electron:
    ...
    if not continue_run:
        ...
        R_array = [orb2c.orbital2c(),
                ...
                orb2c.orbital2c()]
        if (not readOrbitals):
            charge1 = charge -0.3 # ideal for Be 2.45744
            charge2 = charge -2.05 # ideal for Be 3.67137
            L_array[0] = sg.make_NR_starting_guess(position,charge2, mra, prec,comp=2,  n=2, l=0)
            L_array[1] = L_array[0].ktrs(prec)
            L_array[2] = sg.make_NR_starting_guess(position, charge1, mra, prec, comp=2, n=1, l=0)
    ...
    if SWORD_Method:
        ...
        Dirac_array, Fock_matrix = lazy4el.scf_4e_4c(Dirac_array, V_tree, mra, 1e-3, auto_save=False, max_iter=2)
